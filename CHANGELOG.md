...
- Promote the SDK and push extension to the stable 2.8.0 release. The release workflow pins the matching Core archive version and checksum.
...
- Track events before APNs/FCM token registration completes instead of dropping them; the device token is included once available.
- Release the in-app message display gate when a popup is dismissed externally so subsequent messages can be shown.
